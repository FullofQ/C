...
#include <Wire.h>
SoftwareSerial BT(8, 9) ; //設置硬體藍芽腳位
...
boolean comparisonPassKey1(float chaosPassKey){//比對混沌加密後傳輸的USB金鑰與本地金鑰 1
  ...
  if(mistake <= 0.0000000001 && mistake >= -0.0000000001){//透過預設經度判斷金鑰是否正確
    Serial.print("KEY 1 : "); //展示用途，於Serial port 顯示出
    Serial.print(USBKey1,7);
    Serial.print("\t\tGet unChaosKey 1 : ");
    Serial.print(unChaos,7);
    Serial.println("\t\t狀態 : 解鎖成功 ");
...
boolean comparisonPassKey2(float chaosPassKey){//比對混沌加密後傳輸的USB金鑰與本地金鑰 2
  ...
  if(mistake <= 0.00000000001 && mistake >= -0.00000000001){//透過預設經度判斷金鑰是否正確
    Serial.print("KEY 2 : "); //展示用途，於Serial port 顯示出
    Serial.print(USBKey2,7);
    Serial.print("\t\tGet unChaosKey 2 : ");
    Serial.print(unChaos,7);
    Serial.println("\t\t狀態 : 解鎖成功 ");
...
boolean comparisonPassKey3(float chaosPassKey){//比對混沌加密後傳輸的USB金鑰與本地金鑰 3
  ...
  if(mistake <= 0.00000000001 && mistake >= -0.00000000001){//透過預設經度判斷金鑰是否正確
    Serial.print("KEY 3 : "); //展示用途，於Serial port 顯示出
    Serial.print(USBKey3,7);
    Serial.print("\t\tGet unChaosKey 3 : ");
    Serial.print(unChaos,7);
    Serial.println("\t\t狀態 : 解鎖成功 ");
...
void sendIEEE754float(float number){
  IEEE754float* x = (IEEE754float*) ((void*)&number);
    // getVerification(x->A,x->B,x->C,x->D);
    byte(x->A);
    byte(x->B);
    BT.print(byte(x->C));
    BT.print(byte(x->D));
